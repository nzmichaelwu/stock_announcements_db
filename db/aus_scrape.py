...
df_aus_homepage, df_aus_dataroom, df_aus_tradingday = aus_scraper(cfg.urls.aus_homepage, cfg.urls.aus_dataroom, cfg.urls.aus_tradingday, today_s)
print(f"{df_aus_homepage.shape} of records have been scraped from The Australian homepage...")
print(f"{df_aus_dataroom.shape} of records have been scraped from The Australian Data Room...")
print(f"{df_aus_tradingday.shape} of records have been scraped from The Australian Trading Day...")
...
num_rows_aus_dataroom = postgresql_engine.execute("Select count(*) from aus_dataroom").fetchall()[0][0]
print(f'the aus_dataroom table now contains {num_rows_aus_dataroom} rows of data...')

num_rows_aus_tradingday = postgresql_engine.execute("Select count(*) from aus_tradingday").fetchall()[0][0]
print(f'the aus_tradingday table now contains {num_rows_aus_tradingday} rows of data...')
